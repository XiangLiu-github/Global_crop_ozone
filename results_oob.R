...
qsave(cvdata, "data/outputs/oob.qs")
...
mgl_ozone = inner_join(coefs, data) %>%
...
mgl_tmax = list(coefs, data, counter_tmax) %>%
  reduce(inner_join) %>%
...
ggsave("figures/oob.pdf", patch, width = 4, height = 2.5, scale = 3.5)
...
p4 <- data %>%
  ...
  inner_join(summed) %>%
  inner_join(color_tbl) %>%
  ggplot(aes(x = n, y = .lower)) +
  geom_hline(yintercept = 0, linetype = "longdash", color = "red") +
  geom_smooth(method = "lm", se = T) +
  geom_point(aes(color = color), shape = 21, size = 2, stroke = 2) +
  geom_text_repel(aes(label = Item, color = color), family = "Roboto Condensed", box.padding = 0.5, min.segment.length = 1) +
  stat_poly_eq(use_label(c("R2", "P"), sep = "*\"; \"*"), family = "Roboto Condensed") +
  scale_y_continuous("Correlation coeficient") +
  scale_x_log10("Number of observations") +
  scale_color_identity() +
  theme_half_open(font_size = 16, font_family = "Roboto Condensed") +
  background_grid() +
  theme(
...
ggsave("figures/oob_number_R2.pdf", p4, width = 1, height = 1, scale = 7)


# table
t1 = mgl_ozone %>% 
  mutate(aot40 = aot40 * 100) %>% 
  descr(by = ~ Item, Ndistinct = F, higher = F, Qprobs = c(0.01, 0.1, 0.5, 0.9, 0.99)) %>% 
  as_tibble() %>% 
  rename_with(~ str_remove(str_c('\\qty{', .x, '}{\\percent}'), '%'), ends_with('%')) %>% 
  rename(Crop = Group) %>% 
  select(-c(Variable, Class, N)) %>% 
  arrange(factor(Crop, order)) %>% 
  kable(format = 'latex', caption = 'The marginal effects (\\unit{\\percent}) of abating \\qty{1000}{\\ppb\\hour} AOT40 on yields', digits = 8, booktabs = T, label = str_c('suptab', 7), escape = F)  %>%
...
t2 = mgl_tmax %>% 
  ...
  select(-c(Variable, Class, N)) %>% 
  arrange(factor(Crop, order)) %>% 
  kable(format = 'latex', caption = 'The marginal effects (\\unit{\\percent}) of elevating \\qty{2}{\\degreeCelsius} temperature on yields', digits = 8, booktabs = T, label = str_c('suptab', 8), escape = F)  %>%
  kable_styling(latex_options = c('striped', 'hold_position')) %>% 
  landscape() %>% 
  as.character()

str_c(t1, t2, sep = '\n\n\\clearpage\n\n') %>% 
  writeLines("paper/sup/additional_files/bootstrapped_marginal_impacts.tex")

Items = c('Maize', 'Soybean', 'Wheat', 'Rice', 'Sunflower', 'Potato')

str_flatten_comma(str_to_lower(Items), last = ", and ")

mgl_ozone %>%
  filter(Item %in% Items) %>% 
  group_by(Item) %>%
  median_qi(.width = uncertainty[2]) %>%
  arrange(factor(Item, Items)) %>%
  mutate(across(c(aot40, .lower, .upper), ~ round_pettry(.x * 100)),
    num = str_c("\\qty{", aot40, "}{\\percent} (\\qty{", .width * 100, "}{\\percent} CI: \\qtyrange{", .lower, "}{", .upper, "}{\\percent})")
  ) %>%
  pull() %>%
...
mgl_tmax %>%
  ...
  median_qi(.width = uncertainty[2]) %>%
  arrange(factor(Item, Items)) %>%
  mutate(across(c(tmax, .lower, .upper), ~ round_pettry(.x * 100)),
    num = str_c("\\qty{", tmax, "}{\\percent} (\\qty{", .width * 100, "}{\\percent} CI: \\qtyrange{", .lower, "}{", .upper, "}{\\percent})")
  ) %>%
  pull() %>%
  str_flatten_comma(last = ", and ") %>%
  writeLines()
