...
coefs_rtmax <- qread("data/outputs/boot_coefs_tmax_0.1max_removed.qs")
coefs_rtmin <- qread("data/outputs/boot_coefs_tmax_0.1min_removed.qs")
coefs_romax <- qread("data/outputs/boot_coefs_aot40_0.1max_removed.qs")
coefs_romin <- qread("data/outputs/boot_coefs_aot40_0.1min_removed.qs")

p1 <- list(coefs_rtmax, coefs_rtmin, coefs, coefs_romin, coefs_romax) %>%
  ...
  ggplot(aes(x = factor(Item, get_order()), y = aot40, ymax = .upper, ymin = .lower, color = factor(model, labels = c("Removing 10% highest temperature", "Removing 10% lowest temperature", "Full dataset", "Removing 10% lowest ozone", "Removing 10% highest ozone"))))


counter_tmax <- read_rds("data/outputs/tmax/extracted_tmax_count2.rds") %>%
  select(-prop)

p2 <- list(coefs_rtmax, coefs_rtmin, coefs, coefs_romin, coefs_romax) %>%
  map_dfr(function(acoefs) {
  ...
  ggplot(aes(x = factor(Item, get_order()), y = tmax, ymax = .upper, ymin = .lower, color = factor(model, labels = c("Removing 10% highest temperature", "Removing 10% lowest temperature", "Full dataset", "Removing 10% lowest ozone", "Removing 10% highest ozone"))))

patch <- wrap_plots(p1, p2, ncol = 1, guides = "collect") &
  plot_annotation(tag_levels = "a") &
  geom_hline(yintercept = 0, linetype = "longdash", color = "grey30") &
  geom_pointinterval(position = position_dodge(0.75)) &
  scale_y_percent("Change in yield") &
  scale_color_manual(values = c("#EF4343", "#1399B2", "#FCEFB3", "#69BD44", "#F57E20")) &
...
ggsave("figures/robustness_removehigh.pdf", patch, width = 4, height = 2.5, scale = 3.5)
