...
coefs_linear <- qread("data/outputs/boot_coefs_linearFE.qs")

p1 <- list(coefs, coefs_linear) %>%
  ...
  ggplot(aes(x = factor(Item, get_order()), y = aot40, ymax = .upper, ymin = .lower, color = factor(model, labels = c("Perfered specification", "Linear trend fixed effects"))))


counter_tmax <- read_rds("data/outputs/tmax/extracted_tmax_count2.rds") %>%
  select(-prop)

p2 <- list(coefs, coefs_linear) %>%
  map_dfr(function(acoefs) {
  ...
  ggplot(aes(x = factor(Item, get_order()), y = tmax, ymax = .upper, ymin = .lower, color = factor(model, labels = c("Perfered specification", "Linear trend fixed effects"))))
...
ggsave("figures/robustness_linearFE.pdf", patch, width = 4, height = 2.5, scale = 3.5)
